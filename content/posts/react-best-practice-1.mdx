...
### 1.1 Defer Await Until Needed (`await`은 꼭 그 결과가 필요해지는 순간까지 미루세요)
...
### 1.4 Promise.all() for Independent Operations (독립 작업에는 `Promise.all()`을 사용하세요)

- 영향도: 매우 중요 (CRITICAL / 2~10배 개선)

서로 **의존하지 않는** 비동기 작업이면 `Promise.all()`로 **동시에** 실행하세요. 한 줄씩 `await`하면 앞 요청이 끝날 때까지 다음 요청이 시작도 못 해 **순차 대기(워터폴)**가 됩니다.
...
### 1.5 Strategic Suspense Boundaries (`Suspense` 경계 전략)
