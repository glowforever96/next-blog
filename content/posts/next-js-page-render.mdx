...
metaDescription: "Next.js Page Router와 App Router에서 SSR, SSG, ISR을 구현하는 방법과 getServerSideProps·getStaticProps·fetch 옵션 비교를 정리합니다."
...
> 처음 Next.js의 Page Router와 App Router 방식을 공부하면서 가장 자주 잊어버린 개념이 데이터 페칭 방식입니다.
> App Router는 `Server Component` 기반 `fetch()` 중심으로 모든 데이터 페칭 방식이 통합됐지만, 혹여나 Page Router 기반 프로젝트를 접할 일이 있을수도 있어 기록해두었습니다. 추가로 App Router 데이터 패칭 방식, 페이지 생성 전략 등 정리하고 싶은 요소를 정리했습니다.
...
```tsx
// Next.js 15+ 기준: searchParams는 Promise일 수 있음
export default async function ProductsPage({
  ...
  return <div>검색어: {params.keyword}</div>;
}
```
